...
public static class FancyBackground {
    ...
    public static void ModFancyBackgroundParse(ILContext il) {
        ...
        if(cursor.TryGotoNext(MoveType.Before, instr => instr.MatchLdstr("savedata"))){
            ...
            cursor.EmitCall(((Delegate) ParseFancyText).Method);
        }
    }

    private static void ParseFancyText(FancyText text, string s, List<string> vals) {
    ...
    public static void ModFancyBackground(ILContext il, Type EnumeratorType) {
        ...
        cursor.EmitCall(((Delegate) FancyTextboxCheck).Method);
        cursor.Emit(OpCodes.Brtrue, label);
    }

    private static bool FancyTextboxCheck(Textbox self, FancyText.Node node) {
        // Find the node
